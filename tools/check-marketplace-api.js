...
async function main() {
  ...
  try {
    ...
    productIds.push(productToFinish.id, productToCancel.id, productToOffline.id);
    ...
    const stored = await pool
      ...
      .query(`
        ...
        WHERE ExternalId IN (@finishProduct, @cancelProduct);
        SELECT COUNT(*) AS TransactionCount
        FROM dbo.Transactions AS t
        INNER JOIN dbo.Products AS p ON p.ProductId = t.ProductId
        WHERE p.ExternalId IN (@finishProduct, @cancelProduct);
      `);
    ...
    const checks = {
      ...
      productStatusTransitions:
        statusById[productToFinish.id] === "sold" && statusById[productToCancel.id] === "on_sale" && stored.recordsets[1][0].TransactionCount === 2,
      allOwnerStatusesReturned: finalStatuses.has("sold") && finalStatuses.has("on_sale") && finalStatuses.has("offline")
    };
