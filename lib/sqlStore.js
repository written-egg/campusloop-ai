...
function mapManagedProduct(row) {
  return {
    ...
    moderationStatus: row.ModerationStatus || "normal",
    adminOfflineReason: row.AdminOfflineReason || "",
...
async function restoreOwnProduct(userExternalId, productExternalId) {
  const pool = await getPool();
  const result = await pool.request()
    .input("ownerId", sql.NVarChar(40), String(userExternalId || "").trim())
    .input("productId", sql.NVarChar(40), String(productExternalId || "").trim())
    .query(`
      UPDATE p SET StatusName = N'on_sale', UpdatedAt = SYSUTCDATETIME()
      FROM dbo.Products AS p
      INNER JOIN dbo.Users AS u ON u.UserId = p.SellerId
      WHERE p.ExternalId = @productId AND u.ExternalId = @ownerId
        AND p.StatusName = N'offline' AND p.ModerationStatus = N'normal';
      SELECT @@ROWCOUNT AS ChangedRows;
    `);
  if (!result.recordset[0]?.ChangedRows) {
    const product = await getManagedProduct(productExternalId);
    if (!product) throw httpError(404, "商品不存在。" );
    if (product.sellerId !== userExternalId) throw httpError(403, "只能重新上架自己发布的商品。" );
    if (product.moderationStatus === "admin_offline") throw httpError(409, "该商品由管理员下架，需管理员审核恢复。" );
    throw httpError(409, "只有已下架商品可以重新上架。" );
  }
  return getManagedProduct(productExternalId);
}

async function deleteOwnOfflineProduct(userExternalId, productExternalId) {
  const pool = await getPool();
  const transaction = new sql.Transaction(pool);
  await transaction.begin();
  try {
    const locked = await new sql.Request(transaction)
      ...
      .query(`
        SELECT p.ProductId, p.StatusName, u.ExternalId AS SellerExternalId,
          (SELECT COUNT(*) FROM dbo.Transactions t WHERE t.ProductId=p.ProductId) AS TransactionCount
        FROM dbo.Products p WITH (UPDLOCK, HOLDLOCK)
        INNER JOIN dbo.Users u ON u.UserId=p.SellerId
        WHERE p.ExternalId=@productId;
      `);
    const product = locked.recordset[0];
    if (!product) throw httpError(404, "商品不存在。" );
    if (product.SellerExternalId !== userExternalId) throw httpError(403, "只能删除自己发布的商品记录。" );
    if (product.StatusName !== "offline") throw httpError(409, "请先下架商品，再删除记录。" );
    if (product.TransactionCount > 0) throw httpError(409, "该商品已有交易记录，不能删除，只能保持下架状态。" );
    await new sql.Request(transaction).input("productId", sql.Int, product.ProductId).query(`
      UPDATE dbo.Messages SET ProductId=NULL WHERE ProductId=@productId;
      UPDATE dbo.AIReports SET ProductId=NULL WHERE ProductId=@productId;
      UPDATE dbo.RiskLogs SET ProductId=NULL WHERE ProductId=@productId;
      DELETE FROM dbo.Products WHERE ProductId=@productId;
    `);
    await transaction.commit();
    return { id: String(productExternalId), deleted: true };
  } catch (error) {
    await transaction.rollback();
    throw error;
  }
