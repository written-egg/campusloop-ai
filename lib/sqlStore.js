...
async function updateTransactionStatus(userExternalId, transactionId, nextStatus) {
  if (!Number.isInteger(Number(transactionId)) || Number(transactionId) <= 0) throw httpError(400, "交易编号无效。" );
  if (!new Set(["finished", "cancelled"]).has(nextStatus)) throw httpError(400, "不支持的交易状态。" );
  ...
  try {
    ...
    if (trade.TradeStatus !== "pending") throw httpError(409, "只有待处理交易可以变更状态。" );
    if (nextStatus === "finished" && trade.SellerExternalId !== userExternalId) {
      throw httpError(403, "只有卖家可以确认交易完成。" );
    }
    if (nextStatus === "cancelled" && trade.SellerExternalId !== userExternalId && trade.BuyerExternalId !== userExternalId) {
    ...
    await new sql.Request(transaction)
      ...
      .query("UPDATE dbo.Transactions SET TradeStatus = @nextStatus WHERE TransactionId = @transactionId;");
    await transaction.commit();
  } catch (error) {
    await transaction.rollback();
    throw error;
...
async function getAdminOverview() {
  ...
  const result = await pool.request().query(`
    SELECT (SELECT COUNT(*) FROM dbo.Users) AS Users,
           ...
           (SELECT COUNT(*) FROM dbo.Transactions WHERE TradeStatus=N'pending') AS PendingTransactions,
...
async function setAdminUserStatus(adminExternalId, targetExternalId, accountStatus, reason) {
  ...
  try {
    const r = await new sql.Request(tx).input("target",sql.NVarChar(40),targetExternalId).input("status",sql.NVarChar(20),accountStatus).query("UPDATE dbo.Users SET AccountStatus=@status,UpdatedAt=SYSUTCDATETIME() WHERE ExternalId=@target; SELECT @@ROWCOUNT AS n;");
    if (!r.recordset[0].n) throw httpError(404, "用户不存在。" );
...
async function deleteAccount(userExternalId,input) {
  ...
  const pending=await pool.request().input("uid",sql.Int,user.UserId).query("SELECT COUNT(*) AS n FROM dbo.Transactions WHERE (BuyerId=@uid OR SellerId=@uid) AND TradeStatus IN (N'pending',N'paid',N'disputed');"); if(pending.recordset[0].n) throw httpError(409,"账号存在待处理交易，请先完成或取消交易。" );
